% example to test the entity "cplx_weight_sum"
% element-wise complex vector multiplication and sum of results
...
n_ovf = sum(sum(res.cplx.ovf(:,1:LR)));
disp(['Number of overflows detected: ',num2str(n_ovf)]);

% detect incorrect values
n_nan = isnan(res.cplx.rst(:,1:LR)) ...
      + isnan(res.cplx.vld(:,1:LR)) ...
      + isnan(res.cplx.ovf(:,1:LR)) ...
      + isnan(real(res.cplx.data(:,1:LR))) ...
      + isnan(imag(res.cplx.data(:,1:LR)));
n_nan = sum(sum(n_nan));
disp(['Number of incorrect values detected: ',num2str(n_nan)]);

% determine delay
delay = find(res.cplx.vld(:,1),1) - find(res.cplx.vld(:,LR+1),1);
disp(['Multiplier delay [cycles]: ',num2str(delay)]);

% extract valid result values (only result vector without delay reference)
R = res.cplx.data(:,1:LR);
R = R(res.cplx.vld(:,1:LR)==1); % ignore 0 and NAN
R = reshape(R,cycles,[]);

% reference
if LY==1,
  ref = x_data .* repmat(real(y_data),1,LX);
else
  ref = x_data .* real(y_data);
end
% convert to integer
ref = sum((2^15) * ref.').'; 
